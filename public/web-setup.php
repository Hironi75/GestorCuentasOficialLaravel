<?php
        ...
        ?>
        echo '</div>';
        echo '<p><strong>Proyecto está en:</strong> ' . dirname(__DIR__) . '</p>';
        echo '<p><strong>Este archivo está en:</strong> ' . __FILE__ . '</p>';
        ...
        echo '</div>';
        echo '</ol>';
        echo '<li>El Document Root debe apuntar a la carpeta <code>public/</code></li>';
        echo '<li>Verifica que APP_DEBUG=false en .env</li>';
        echo '<li>Si necesitas ejecutar migraciones, usa el archivo <code>web-migrate.php</code></li>';
        echo '<li><strong>ELIMINA este archivo (web-setup.php) INMEDIATAMENTE</strong></li>';
        echo '<ol>';
        ...
        echo '</div>';
            ...
            echo '<p class="error">✗ No se puede conectar a la base de datos</p>';
                ...
                echo '<p class="info">ℹ Para ejecutar migraciones, usa el archivo <code>web-migrate.php</code></p>';
        ...
        echo '</div>';
            ...
            $hasErrors = true;
            echo '</div>';
            echo '4. Asegúrate de que esté en la ubicación correcta';
            echo '3. Sube la carpeta <code>public/build/</code> al hosting via FTP o Git<br>';
            echo '2. Verifica que existe: <code>public/build/manifest.json</code><br>';
            echo '1. En tu PC ejecuta: <code>npm run build</code><br>';
            echo '<strong>ACCIÓN REQUERIDA:</strong><br>';
            echo '<div class="alert alert-danger">';
            echo '<p>Buscado en: ' . $manifestPath . '</p>';
            echo '<p class="error">✗ public/build/manifest.json NO EXISTE</p>';
        } else {
            }
                $hasErrors = true;
                echo '</div>';
                echo 'Asegúrate de subir la carpeta completa <code>public/build/</code> desde tu PC.';
                echo '<strong>❌ ERROR:</strong> Algunos archivos de assets no existen físicamente.<br>';
                echo '<div class="alert alert-danger">';
            if (!$allAssetsExist) {
                ...
                if (!file_exists(__DIR__ . '/build/' . $value['file'])) {
            ...
            echo '</ul>';
                ...
                echo '<li><span class="' . $color . '">' . $status . '</span> ' . $key . ' → ' . $value['file'] . '</li>';
                $color = $exists ? 'success' : 'error';
                $status = $exists ? '✓' : '✗';
                $exists = file_exists($assetFile);
                $assetFile = __DIR__ . '/build/' . $value['file'];
        ...
        echo '<div class="step">';
        // PASO 4: Verificar assets de Vite
        ...
        echo '</div>';
            ...
            echo '<p class="error">✗ Archivo .env NO existe en: ' . $envPath . '</p>';
                ...
                $hasErrors = true;
                echo '</div>';
                echo '<strong>❌ ERROR:</strong> APP_KEY no está configurada. Ejecuta: <code>php artisan key:generate</code>';
                echo '<div class="alert alert-danger">';
            if (!env('APP_KEY')) {
        ...
        echo '</div>';
        echo '<p class="success">✓ Rutas correctas</p>';
        echo '<p><strong>Directorio del proyecto:</strong> ' . dirname(__DIR__) . '</p>';
        echo '<p><strong>Este archivo está en:</strong> ' . __DIR__ . '</p>';
        echo '<h3>0. Verificando ubicación del archivo</h3>';
        echo '<div class="step">';
...
// Subir un nivel desde public/ para acceder a vendor y bootstrap

 */
 * 3. ELIMINA este archivo después de usar (SEGURIDAD)
 * 2. Visita: https://tudominio.com/web-setup.php
 * 1. Coloca este archivo en: public/web-setup.php
 * USO:
 *
 * que están un nivel arriba.
 * Este archivo está diseñado para estar en public/ y acceder a los recursos del proyecto
 *
 * WEB-SETUP.PHP - Setup accesible desde public/
/**
